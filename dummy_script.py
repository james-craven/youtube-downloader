...
"padding: 4px;")
    ...
    def download(self, progress_callback):
        ...
        for link in self.youtubeList:
            link.register_on_progress_callback(progress_func)
            link.register_on_complete_callback(completed_func)
            try:
                print(link.streams.filter(file_extension='mp4').first().default_filename)
                link.streams.filter(file_extension='mp4').first().download()
            except Exception as e:
                print(f'Exception: {link.title}\n{e}')
                pass
    ...
    def addProgressBars(self):
        ...
        for i in range(s):
            ...
            try:
                ...
                globals()[link] = ProgressBar(minimum=0, maximum=100, textVisible=True, text="Downloading...", format="Downloading...%p%")
